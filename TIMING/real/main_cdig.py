...
class FourierIG(CrossDomainIG):
    def __init__(self, 
        ...
        self.initialize_domain(FourierDomain, device = device, dtype = dtype, time_dimension = 1)
...
def Fourier(x):
    return th.fft.fft(x, dim = 1)

def InverseFourier(z):
    return th.fft.ifft(z, dim = 1)

def CDIG(model, sample, device):
    data = sample.to(device)
    output = model(data.float())
    output_channel = np.argmax(output.detach().cpu().numpy(), axis = -1)

    fourier_saliency_map = []
    for j in range(output_channel.size):
        ig = FourierIG(model = model, n_iterations = 20, device = device, output_channel = output_channel[j])
        ig_data = data[j : j + 1, ...].detach().cpu().numpy()
        baseline = np.zeros_like(ig_data)
        fourier_saliency_map.append(ig.run(ig_data, baseline).cpu())
    
    fourier_saliency_map = th.cat(fourier_saliency_map, dim = 0)
    return fourier_saliency_map

def CDIG_batched(model, sample, device):
    data = sample.to(device)
    output = model(data.float())
...
def main(
    ...
    transformed_model = TransformModelWrapper(time_model = classifier, inverse_transform = InverseFourier)
    ...
    with lock:
        ...
        x_test_frequency = Fourier(x_test)
    ...
    print("Running cdig")
    if "cdig" in explainers:
        attributions = []
        for sample, target in tqdm(test_loader):
            attributions.append(CDIG_batched(classifier, sample, device))

        attr["cdig"] = th.cat(attributions)

    # Compute x_avg for the baseline
    x_avg = x_test.mean(1, keepdim=True).repeat(1, x_test.shape[1], 1)
    x_avg = th.fft.fft(x_avg, dim=1)  # Or simply: x_avg.to(dtype=torch.complex64)
