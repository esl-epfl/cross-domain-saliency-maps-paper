...
def compute_time_attr(model, X):
    N = X.shape[1]
    ig = IGbatch(model, X).numpy()[..., 0]
    return ig
...
def evaluate_fourier_stability(model, X_test, xf_bpm, rng,
    ...
    attr_clean = compute_time_attr(model, X_test)
    for snr_db in snr_levels:
        ...
        for r in tqdm(range(n_repeats)):
            ...
            attr_noisy = compute_time_attr(model, X_noisy)
...
os.makedirs('./results/time_perturbation_test', exist_ok=True)
